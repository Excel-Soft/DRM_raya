...
export function registerGmPoolRoutes(app: Express) {
  ...
  router.get("/gm-pool/:id/invoices", requireGmSalesActionPermission(GM_SALES_ACTION_KEYS.GM_DELETE), async (req, res) => {
    try {
      if (!req.user) return res.status(401).json({ error: "Not authenticated" });
      const { id } = req.params;

      const gmRes = await pool.query(
        `SELECT id, created_by FROM drm.gm_entries WHERE id = $1 AND coalesce(is_deleted, false) = false`,
        [id]
      );
      if (!gmRes.rowCount) return res.status(404).json({ error: "GM entry not found" });

      const role = normalizeRole((req.user as any).activeRoleId || req.user.roleId || "");
      const isOrgWide = role === ROLES.ADMIN || role === ROLES.SUPER_HOD;
      if (!isOrgWide && gmRes.rows[0].created_by !== req.user.userId) {
        return res.status(404).json({ error: "GM entry not found or not owned by you" });
      }

      const invRes = await pool.query(
        `SELECT id, invoice_number, invoice_type, status, COALESCE(amount,0)::numeric AS amount,
                currency, project_name, company_name, auto_generated, created_at
      ...
      return res.status(500).json({ error: "Failed to fetch linked invoices" });
    }
  });
  ...
  router.delete("/gm-pool/:id", requireGmSalesActionPermission(GM_SALES_ACTION_KEYS.GM_DELETE, { auditUnauthorizedAttempt: true }), async (req, res) => {
    ...
    try {
      ...
      const checkRes = await client.query(
        `SELECT approval_status, hod_status, hod_approved_at, final_status, status, created_by
         FROM drm.gm_entries WHERE id = $1 AND coalesce(is_deleted, false) = false FOR UPDATE`,
        [id]
      );
      ...
      const finalStat = (gmRow.final_status || "").toLowerCase().trim();
      const statusCol = (gmRow.status || "").toLowerCase().trim();
      // approval_status === 'rejected_by_hod' is the authoritative "was rejected"
      // signal; hod_status / hod_approved_at can be stale leftovers from an EARLIER
      // approval that was later reversed by a resubmit-then-reject cycle (neither
      // reject endpoint clears them) — mirrors the frontend's same precedence rule
      // (gm-pool-add-gm.tsx isHodApproved/isRejected).
      const isRejected =
        appStatus.includes("rejected") || statusCol.includes("rejected") ||
        hodStat.includes("rejected") || finalStat.includes("rejected");
      const isHodApproved =
        !isRejected &&
        (appStatus === "approved" ||
      ...
      const role = normalizeRole((req.user as any).activeRoleId || req.user.roleId || "");
      const isOrgWide = role === ROLES.ADMIN || role === ROLES.SUPER_HOD;
      if (!isOrgWide && gmRow.created_by !== req.user.userId) {
        await client.query("ROLLBACK");
        return res.status(404).json({ error: "GM entry not found or not owned by you" });
      }

      // product_posting_invoices has no soft-delete column, so linked invoices are
      // hard-deleted here. drm.projects.invoice_id has no live FK to this table
      // (declared in schema.ts/migrations but never applied to this DB — verified
      // against pg_constraint), so this cannot throw a FK violation.
      ...
      await client.query("COMMIT");
      return res.json({ success: true, message: "Entry deleted", invoicesDeleted: invRes.rowCount });
    } catch (err) {
      try { await client.query("ROLLBACK"); } catch {}
