...
export class DashboardController {
  ...
  // Revenue series
  static async getRevenueSeries(req: Request, res: Response) {
    try {
  ...
  static async getOrdersSeries(req: Request, res: Response) {
    try {
      const parsed = summaryQuerySchema.parse(req.query);
      const { from, to } = parseDateRange(parsed);
      const bucket = parsed.bucket ?? "daily";
  ...
  static async getSalesByChannel(req: Request, res: Response) {
    try {
      const parsed = summaryQuerySchema.parse(req.query);
      const { from, to } = parseDateRange(parsed);
      const region = parsed.branchId || null;
  ...
  static async getCurrentMonthTrend(req: Request, res: Response) {
    try {
      if (!req.user) return res.status(401).json({ success: false, message: "Not authenticated" });
      const now = new Date();
  ...
  static async getDailyTeamMeeting(req: Request, res: Response) {
    try {
      if (!req.user) return res.status(401).json({ success: false, message: "Not authenticated" });
      let from: Date;
      if (req.query.date) {
  ...
  static async startTeamMeeting(req: Request, res: Response) {
    try {
      if (!req.user) return res.status(401).json({ success: false, message: "Not authenticated" });
  ...
  static async endTeamMeeting(req: Request, res: Response) {
    try {
      if (!req.user) return res.status(401).json({ success: false, message: "Not authenticated" });
      const targetUserId = req.params.userId;
  ...
  static async getActiveTeamMeetings(req: Request, res: Response) {
    try {
      if (!req.user) return res.status(401).json({ success: false, message: "Not authenticated" });
  ...
  static async getTeamQueuePerformance(req: Request, res: Response) {
    try {
      if (!req.user) return res.status(401).json({ success: false, message: "Not authenticated" });
      const period = String(req.query.period ?? "MONTH").toUpperCase();
  ...
  static async getTeamWorkPerformance(req: Request, res: Response) {
    try {
      if (!req.user) return res.status(401).json({ success: false, message: "Not authenticated" });
  ...
  static async getChartData(req: Request, res: Response) {
    try {
      ...
      const countSql = `
         ...
         FROM drm.opportunities op
         INNER JOIN drm.customers c ON c.id = op.customer_id
         WHERE lower(op.stage::text) = lower($1)
           AND coalesce(op.is_deleted, false) = false
           ${searchFilter}
           ${ownershipFilter}
           ${dateFilter}
