...
type RowData = { company: string; person: string; service: string; priority: string; status: string; detail: string; date: string; };
                    ...
                    <Table>
                        ...
                        <TableBody>
                            {filteredData.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={8} className="py-4 text-center text-slate-500 text-[13px] border-b border-slate-200 dark:text-zinc-400 dark:border-zinc-800">
                                ...
                                filteredData.map((row, idx) => (
                                    <TableRow key={idx} className="border-b border-slate-200 dark:border-zinc-800">
                                        ...
                                        {columns.service && <TableCell className="py-2 text-[13px]">{row.service}</TableCell>}
                                        {columns.priority && <TableCell className="py-2 text-[13px]">{row.priority}</TableCell>}
                                        {columns.status && <TableCell className="py-2 text-[13px]">{row.status}</TableCell>}
                                        {columns.detail && <TableCell className="py-2 text-[13px]">{row.detail}</TableCell>}
