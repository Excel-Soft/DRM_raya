...
export default function PostingDataLinkReport() {
  ...
  const { data: reportData, isLoading } = useQuery<any>({
    queryKey: ["/api/posting-data/link-report", fetchParams],
    queryFn: async () => {
      ...
      try {
        const res = await apiRequest("GET", `/api/posting-data/link-report?${params}`);
        ...
        return {
          details: [
            { 
              id: "2021-07-13 17:08:25", 
              company: "2021-07-13 17:08:25", 
              links: "2021-07-13 17:08:25", 
              date: "2021-07-13T17:08:25.000Z" 
            }
          ]
        };
      }
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-zinc-800">
                {isLoading ? (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-500">Loading data...</td>
                  </tr>
                ) : details.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="py-8 text-center text-slate-500">No entries found</td>
                  </tr>
                ) : (
                  details.map((item: any, index: number) => (
                    <tr key={index} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/50 transition-colors">
                      ...
                      <td className="py-4 px-4 text-slate-600 dark:text-zinc-400">{item.id || "-"}</td>
                      <td className="py-4 px-4 text-slate-600 dark:text-zinc-400">{item.company || "-"}</td>
                      <td className="py-4 px-4 text-slate-600 dark:text-zinc-400">{item.links || "-"}</td>
