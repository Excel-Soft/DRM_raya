...
type RowData = { company: string; person: string; project: string; grade: string; status: string; date: string; };
...
export default function ServiceVasDocumentList() {
    ...
    const handleCopy = () => {
        const headers = ["#", ...Object.keys(columns).filter(k => columns[k as keyof typeof columns])].map(k => k.charAt(0).toUpperCase() + k.slice(1)).join("\t");
        const rows = filteredData.map((row, idx) => {
            ...
            if (columns.project) rowData.push(row.project);
                    ...
                    <div className="flex bg-[#64748b] rounded-[4px] overflow-hidden">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuCheckboxItem checked={columns.project} onCheckedChange={(v) => setColumns(p => ({ ...p, project: v }))}>Project</DropdownMenuCheckboxItem>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-[#d1fae5] border-b border-slate-200 hover:bg-[#d1fae5] dark:border-zinc-800 dark:bg-zinc-900 dark:hover:bg-zinc-800">
                                ...
                                {columns.project && <TableHead className="text-[13px] font-bold text-slate-800 py-2.5 dark:text-zinc-100">Project</TableHead>}
                            ...
                            {filteredData.length === 0 ? (
                                ...
                                filteredData.map((row, idx) => (
                                    <TableRow key={idx} className="border-b border-slate-200 dark:border-zinc-800">
                                        ...
                                        {columns.project && <TableCell className="py-2 text-[13px]">{row.project}</TableCell>}
