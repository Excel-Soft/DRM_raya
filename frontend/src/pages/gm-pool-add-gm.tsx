...
export default function GmPoolAddGm() {
  ...
  const { data: deleteInvoicesData, isLoading: deleteInvoicesLoading } = useQuery<{
    invoices: Array<{
      id: string;
    ...
    queryKey: ["gm-pool-delete-invoices", deleteId],
    queryFn: async () => {
      const res = await fetch(`/api/gm-pool/${deleteId}/invoices`, {
        headers: { ...getAuthHeader() },
        credentials: "include",
      });
  ...
  const deleteInvoices = deleteInvoicesData?.invoices ?? [];
    ...
    <div className="flex-1 overflow-auto p-6 space-y-6" data-testid="page-gm-pool-add-gm">
                          ...
                          <TableCell className="whitespace-nowrap">
                            <div className="flex items-center gap-1">
                              ...
                              {(() => {
                                ...
                                // A definitive rejection (approval_status === 'rejected_by_hod', the
                                // field every reject action authoritatively writes) always wins over
                                // hod_status / hod_approved_at — those can be stale leftovers from an
                                // EARLIER approval that was later reversed by a resubmit-then-reject
                                // cycle; gm-pool-routes.ts's reject endpoints never clear them.
                                const isHodApproved =
                                  !isRejected &&
                                  (hodTone === "ok" ||
      ...
      <Dialog open={!!deleteId} onOpenChange={(open) => !open && setDeleteId(null)}>
        <DialogContent className="max-w-md">
          ...
          {deleteInvoicesLoading ? (
            <div className="py-4 text-sm text-gray-500 flex items-center gap-2">
              <Loader2 className="h-4 w-4 animate-spin" /> Checking linked invoices...
            </div>
          ) : deleteInvoices.length > 0 ? (
            <div className="py-2 space-y-2">
              <p className="text-sm font-semibold text-destructive">
                Deleting this GM will also permanently delete {deleteInvoices.length} linked
                invoice{deleteInvoices.length > 1 ? "s" : ""}. This cannot be undone.
              </p>
              <div className="border rounded-md divide-y max-h-48 overflow-y-auto text-xs">
                {deleteInvoices.map((inv) => (
                  <div key={inv.id} className="flex items-center justify-between px-2 py-1.5 gap-2">
                    <div className="min-w-0">
                      <div className="font-mono font-semibold truncate">{inv.invoiceNumber || inv.id}</div>
                      <div className="text-gray-500 truncate">{inv.projectName || inv.invoiceType || "-"}</div>
                    </div>
                    <div className="text-right shrink-0">
                      <div>{inv.currency || "USD"} {inv.amount}</div>
                      <div className="text-gray-500">{inv.status}</div>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <DialogFooter className="gap-2">
            ...
            <Button
              ...
              disabled={deleteMutation.isPending || deleteInvoicesLoading}
            >
