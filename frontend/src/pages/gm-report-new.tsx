...
export default function GmReportNew() {
  ...
  const createMutation = useMutation({
    mutationFn: async () => {
      ...
      const res = await apiRequest("POST", "/api/gm-reports", body);
      return res.json();
    },
    onSuccess: () => {
      toast({ title: "GM Report created" });
      queryClient.invalidateQueries({ queryKey: ["/api/reports", "gm"] });
      queryClient.invalidateQueries({ predicate: (q) => Array.isArray(q.queryKey) && q.queryKey.includes("/api/reports") });
      navigate("/reports/gm");
    },
    onError: (err: any) => {
      toast({ title: "Error", description: err?.message || "Failed to create GM report", variant: "destructive" });
    },
  });
  ...
  return (
    <div className="p-6">
      <Card className="max-w-3xl mx-auto">
        <CardHeader>
          <CardTitle>Create GM Report</CardTitle>
        </CardHeader>
        <CardContent>
