...
export default function ServiceBvDocumentList() {
    ...
    const { data, isLoading } = useQuery<ListResponse>({
        queryKey: ["/api/service/documents", "BV", page, searchTerm],
    ...
    const invalidate = () => queryClient.invalidateQueries({ queryKey: ["/api/service/documents", "BV"] });
    ...
    const handleCopy = () => {
        const headers = ["#", "Company", "Name", "Package", "Status", "Due Date", "Uploaded By"].join("\t");
        const body = rows.map((row, idx) =>
            [idx + 1, row.companyName || "", row.name || "", row.packageName || "", row.verificationStatus, formatDate(row.dueDate), row.uploadedByName || ""].join("\t")
        ).join("\n");
        ...
        <div className="bg-[#f8fafc] font-sans p-4 min-h-screen printable-area dark:bg-zinc-950">
                    ...
                    <div className="flex bg-[#64748b] rounded-[4px] overflow-hidden">
                        ...
                        <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                                ...
                                <DropdownMenuCheckboxItem checked={columns.uploadedBy} onCheckedChange={(v) => setColumns(p => ({ ...p, uploadedBy: v }))}>Uploaded By</DropdownMenuCheckboxItem>
                            </DropdownMenuContent>
                        </DropdownMenu>
                    </div>
                    ...
                    <Table>
                        <TableHeader>
                            <TableRow className="bg-[#d1fae5] border-b border-slate-200 hover:bg-[#d1fae5] dark:border-zinc-800 dark:bg-zinc-900 dark:hover:bg-zinc-800">
                                ...
                                {columns.uploadedBy && <TableHead className="text-[13px] font-bold text-slate-800 py-2.5 dark:text-zinc-100">Uploaded By</TableHead>}
                            ...
                            {isLoading ? (
                                ...
                                rows.map((row, idx) => (
                                    <TableRow key={row.id} className="border-b border-slate-200 dark:border-zinc-800">
                                        ...
                                        {columns.package && <TableCell className="py-2 text-[13px]">{row.packageName || "—"}</TableCell>}
                                        ...
                                        {columns.uploadedBy && <TableCell className="py-2 text-[13px]">{row.uploadedByName || "—"}</TableCell>}
            ...
            <Dialog open={formOpen} onOpenChange={(o) => { if (!o) closeForm(); }}>
                <DialogContent className="max-w-lg">
                    <DialogHeader>
                        <DialogTitle>{editing ? "Edit BV Document" : "New BV Document"}</DialogTitle>
                    </DialogHeader>
