...
export default function UpcomingProjectPage() {
    ...
    const { data: response, isLoading } = useQuery<any>({
        queryKey: ['/api/hod/projects/upcoming'],
    });

    const upcomingProjects = response?.data || [];

    const filteredData = useMemo(() => {
        if (!searchQuery.trim()) return upcomingProjects;
        const lowerQuery = searchQuery.toLowerCase();
        return upcomingProjects.filter((row: any) =>
    ...
    const generateTooltip = (label: string, row: any) => {
        ...
        return `Create : ${createDate} | Status : ${actionDate} | Dead Line : ${deadLineDate} | DAYS : ${row.days || 0}`;
    };
        ...
        <div className="p-4 sm:p-6 min-h-screen bg-[#f4f6f9] font-sans dark:bg-zinc-950">
            <div className="mb-6 flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                    <h1 className="text-[17px] font-bold text-[#495057] uppercase tracking-wide dark:text-zinc-400">UPCOMING PROJECTS</h1>
                    ...
                    <table className="w-full border-collapse text-left whitespace-nowrap min-w-[1200px]">
                        <thead>
                            <tr className="bg-[#f8f9fa] border-b border-gray-100 dark:bg-zinc-900 dark:border-zinc-800">
                                ...
                                <th className="px-4 py-4 text-[13px] font-bold text-[#495057] text-center dark:text-zinc-400">Days left</th>
                            </tr>
                        </thead>
                            ...
                            {isLoading ? (
                                ...
                                filteredData.map((row: any, idx: number) => (
                                    <tr key={idx} className="hover:bg-gray-50/50 dark:hover:bg-zinc-800 transition-colors">
                                        ...
                                        <td className="px-3 py-4 text-[13px] text-gray-600 text-center font-bold text-[#0f8c5b] dark:text-zinc-300">{row.days || 0}</td>
                                    </tr>
                                ))
                            )}
                        </tbody>
