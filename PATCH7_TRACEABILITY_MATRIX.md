...
| DB-001 | P0 | DB migration / schema sync | P7 | — | `drizzle.config.ts`; `ensureDbOnce` (`index.ts:76`); `db/ensure.ts` | `drm` schema | Reliable schema application | CODE runtime ensure DDL; LIVE 528 tables | fix FK; restore `db:push` | 1/6 | schema parity | applied | Partial |
